...
def run_LAmbDA(gamma, delta, tau, prc_cut, bs_prc, do_prc, hidden_feats, lambda1, lambda2, lambda3):
	...
	layer2=add_layer(layer1,hidden_feats,hidden_feats,activation_function=tf.sigmoid,dropout_function=True,lambda1=lambda1,keep_prob1=do_prc)
	layer3=add_layer(layer2,hidden_feats,hidden_feats,activation_function=tf.sigmoid,dropout_function=True,lambda1=lambda1,keep_prob1=do_prc)
	predict=add_layer(layer3,hidden_feats,output_feats,activation_function=tf.nn.softmax,dropout_function=False,lambda1=lambda1)
	...
	Ct = tf.transpose(tf.matmul(tf.transpose(layer3),ys))/tf.reshape(tf.reduce_sum(tf.transpose(ys+0.01),1),(-1,1));
	...
	if prt:
		...
		sio.savemat('preds117_cv' + str(cv) + '.mat', {'preds': blah});
		sio.savemat('truth117_cv' + str(cv) + '.mat', {'labels': Y[test, :]});
		sio.savemat('hidden117_cv' + str(cv) + '.mat', {'hidden': blah2});
