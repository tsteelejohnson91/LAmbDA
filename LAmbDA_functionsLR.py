...
def run_LAmbDA(gamma, delta, tau, prc_cut, bs_prc, do_prc, lambda1):
	...
	#layer1=add_layer(xs,input_feats,hidden_feats,activation_function=tf.sigmoid,dropout_function=True,lambda1=3.5)
	predict=add_layer(xs,input_feats,output_feats,activation_function=tf.nn.softmax,dropout_function=False,lambda1=lambda1, keep_prob1=do_prc)
	...
	train_step = tf.train.AdamOptimizer(learning_rate=0.01,epsilon=1e-8).minimize(loss1)
	...
	for i in range(iter + 1):
		if i <= 1000:
			...
			if i % 10 == 0:
				print(str(sess.run(loss1, feed_dict=tensor_train)) + ' ' + str(sess.run(loss1, feed_dict=tensor_test)));
			elif i % 50 == 0:
...
for i in range(1,11):
	...
	prt = False
	opt_params = None
	opt_params, _, _ = opt.minimize(run_LAmbDA,solver_name='sobol', gamma=[0.8,1.2], delta=[0.05,0.95], tau=[1.0,2.0], prc_cut=[20,50], bs_prc=[0.2,0.6], do_prc=[0.5,1], lambda1=[0,5], num_evals=50)
	prt = True
	train = perm[0:train_samp+test_samp+1]
	test = val
	err = run_LAmbDA(opt_params['gamma'], opt_params['delta'], opt_params['tau'], opt_params['prc_cut'], opt_params['bs_prc'], opt_params['do_prc'], opt_params['lambda1'])
	tf.reset_default_graph();
